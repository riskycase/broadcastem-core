...
describe('When trying to start the app', () => {
	...
	it('it should reject when a path is invalid', (done) => {
		...
		.catch(err => {
			/dummy[/\\]I[/\\]dont[/\\]exist does not exist! \(at .*dummy[/\\]dummy-list-invalid.txt at 3\)/.test(err).should.equal(true);
			done();
		});
	});
	...
	it('it should reject when a path is invalid even in a long list', (done) => {
		...
		.catch(err => {
			/dummy[/\\]doesnt[/\\]exist does not exist! \(at .*dummy[/\\]dummy-list-long-invalid.txt at 53\)/.test(err).should.equal(true);
			done();
		});
	});
