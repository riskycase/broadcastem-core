...
function readLines(listPath) {
	...
	return new Promise((resolve, reject) => {
		...
		readInterface.on('line', line =>  {
			...
			if( !isPathPresent(directFiles, line) && !isPathPresent(listFiles, line) ) {
				...
				// Informs in the error message which path doesn't exist
					...
					reject(`${line} does not exist! (at ${path.resolve(process.cwd(), listPath)} at ${lineNumber})`);
			}
			lineNumber++;
		});
...
function getSize(givenPath) {
	...
	return new Promise((resolve, reject) => {
		fileHandler.stat(givenPath)
		.then(stats => {
			if(!stats.isDirectory())
				setImmediate(resolve, stats.size);
			else 
				fileHandler.readdir(givenPath)
				.then(children => children.map(child => path.resolve(givenPath, child)))
				.then(paths => Promise.all(paths.map(getSize)))
				.then(sizes => sizes.reduce((total, size) => total + size, stats.size))
				.then(resolve);
		});
	});
}
...
function initFiles(options) {
	...
	directFiles = options.files.filter((value, index, self) => self.indexOf(value) === index).map(fileMaker);
